//! ConfigSummary 启动期编码为紧凑字节序列 — framework 机制功能
//!
//! ## DECISION-J 归属反转记录 (2026-09-12)
//!
//! 原策略代码于 2026-06-17 从 `framework::config::boot_image` 迁移至 services, 本
//! 文件仅 re-export。按"机制持有的数据结构/常量归 framework"统一判据反转：
//! `encode_boot_image` 被 framework config `init()` 机制直接调用 (崩溃后取证编码),
//! 且依赖闭包全在 framework 内 (`get_config_summary` + `IrqSpinLock`) — 属机制
//! 功能, 迁回。
//!
