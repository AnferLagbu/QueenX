...
fn nestfs_restore() {
    ...
    nestfs.initialized.store(false, Ordering::Release);
    nestfs.mounted.store(false, Ordering::Release);
    nestfs.spa.init("queenx-pool");
    nestfs.setup_zil_datasets();
    nestfs.root_ds_id.store(0, Ordering::Release);
    nestfs.current_dir.store(HV_DMU_OBJ_ROOT, Ordering::Release);
    nestfs.mounted.store(true, Ordering::Release);
    nestfs.initialized.store(true, Ordering::Release);
    crate::slog_info!(FS, "[NestFS] Recovery: domain restored");
}
fn nestfs_reset() {
    crate::slog_warn!(FS, "[NestFS] Recovery: domain hard reset");
    // J-03 (2026-09-08, G-10 方案 C): 空壳实装 — 显式重建 objset, 供栏栈硬重置恢复路径
    get_nestfs().reset();
}

pub const NESTFS_MAX_FDS: usize = 256;
...
pub struct NestfsData {
    ...
    pub zil: NestZil,
    pub fds: Mutex<[NestfsFd; NESTFS_MAX_FDS]>,
    ...
    pub mode: AtomicU8,
    /// 已发现的 QueenX/NestFS 磁盘驱动器列表 (`drive_id`, `partition_start_lba`)
...
static NESTFS_DATA: OnceCell<NestfsData> = OnceCell::new();

pub fn get_nestfs() -> &'static NestfsData {
    NESTFS_DATA.get_or_init(|slot| {
...
impl NestfsData {
    ...
    fn scan_all_drives(&self) -> Vec<(u8, u32)> {
        ...
        for drive in 0..8u8 {
            ...
            // 检查 QueenX/NestFS 签名扇区 (LBA 2046)
            let part_start =
        ...
        crate::slog_info!(
            FS,
            "[NestFS] scan: {} block device(s) in registry, {} drive(s) available",
    ...
    pub fn init(&self) {
        ...
        // 显式重建改经 reset() (栏栈恢复钩子 nestfs_reset/nestfs_restore 调用).
        if self.is_initialized() {
            crate::slog_info!(
                FS,
                "[NestFS] init() skipped: already initialized (idempotent, data preserved)"
            );
            return;
        }
        crate::slog_info!(FS, "[NestFS] Initializing...");

        // Step 1: 扫描所有块设备, 发现 QueenX/NestFS 磁盘
        ...
        if has_any_disk {
            // Step 2: 尝试从已发现的磁盘挂载 NestFS (uberblock 验证)
            ...
            if !mounted_any {
                // Step 3: 格式化第一个磁盘 (全新 NestFS)
                crate::slog_warn!(
                    FS,
                    "[NestFS] No valid uberblock found, formatting first drive..."
                );
            ...
            crate::slog_info!(
                FS,
                "[NestFS] Initialized: pool=queenx-pool (disk, {} drive(s))",
                discovered.len()
            );
        } else {
    ...
    pub fn reset(&self) {
        ...
        self.mounted.store(true, Ordering::Release);
        self.initialized.store(true, Ordering::Release);
        crate::slog_info!(FS, "[NestFS] Reset complete: objset rebuilt");
    }
    ...
    pub fn format_drive(&self, drive_id: u8, part_start: u32) {
        ...
        crate::slog_info!(
            FS,
            "[NestFS] FORMAT: Writing fresh NestFS v2 to disk (partition @LBA {})...",
            part_start
        );
        ...
        crate::slog_info!(FS, "[NestFS] FORMAT: Complete");
    }
    ...
    pub fn hotplug_add_disk(&self, drive: u8) -> bool {
        if !block::hdd_is_present(drive) {
            crate::slog_warn!(FS, "[NestFS] HOTPLUG: drive not present, skip");
            return false;
        }
        ...
        // 检查是否已存在该驱动
        {
            ...
            if discovered.iter().any(|(d, _)| *d == drive) {
                crate::slog_info!(FS, "[NestFS] HOTPLUG: drive already known, skip");
                return false;
            }
        }
        ...
        crate::slog_info!(FS, "[NestFS] HOTPLUG: disk added (drive={})", drive);
        true
    }
    ...
    pub fn hotplug_remove_disk(&self, drive: u8) -> bool {
        ...
        crate::slog_warn!(
            FS,
            "[NestFS] HOTPLUG: disk not found in vdevs (drive={})",
            drive
        );
        false
    }
    ...
    pub fn mount_drive(&self, drive_id: u8, part_start: u32) -> bool {
        ...
        crate::slog_info!(
            FS,
            "[NestFS] MOUNT: Reading uberblock from disk (partition @LBA {})...",
            part_start
        );
        let ub = if let Some(u) = self.spa.read_uberblock_from_disk() {
            u
        } else {
            crate::slog_warn!(FS, "[NestFS] MOUNT: No valid uberblock found");
            return false;
        };
        crate::slog_info!(FS, "[NestFS] MOUNT: Valid uberblock found (txg={})", ub.txg);
        {
        ...
        }
        {
            ...
            if root_bp.is_null() {
                ...
                crate::slog_info!(FS, "[NestFS] MOUNT: Restored dataset from uberblock");
            } else {
                let datasets = self.datasets.lock();
                datasets[0].init(0);
            }
    ...
    pub fn read(&self, fd: u32, buf: &mut [u8], count: u32) -> i32 {
        let (obj_id, offset, pwm) = {
            ...
            let idx = fd as usize;
            if idx >= NESTFS_MAX_FDS || !fds[idx].used {
                return KernelError::InvalidArgument.as_i32();
            }
    ...
    fn serialize_dataset_metadata(&self, txg: u64) -> Option<NestBlockPointer> {
        ...
        if objects.len() > MAX_SERIALIZE_OBJECTS || dir_entries.len() > MAX_SERIALIZE_ENTRIES {
            crate::slog_err!(
                FS,
                "[NestFS] serialize: object/entry count exceeds safety limit"
            );
            return None;
        }
    ...
    fn deserialize_dataset_metadata(&self, bp: &NestBlockPointer) -> bool {
        ...
        if obj_count > MAX_DESERIALIZE_OBJECTS || zap_count > MAX_DESERIALIZE_ENTRIES {
            crate::slog_err!(
                FS,
                "[NestFS] deserialize: count exceeds safety limit, possible corruption"
            );
            return false;
        }
        ...
        if (buf.len() as u64) < expected_min {
            crate::slog_err!(
                FS,
                "[NestFS] deserialize: buffer too small for declared counts"
            );
            return false;
        }
        ...
        let mut off = 32usize;
        {
            ...
            for _ in 0..zap_count {
                ...
                if name_len > MAX_NAME_LEN {
                    crate::slog_err!(
                        FS,
                        "[NestFS] deserialize: name length exceeds limit, possible corruption"
                    );
                    return false;
                }
    ...
    /// 获取 `NestFS` 池统计 (allocs, frees, reads, writes)
