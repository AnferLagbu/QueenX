...
- **S-9. map 路径过度 shootdown（门槛 7 实测发现，须修复）**
  ...
  - 方案：**待分类调研后填写**。`flush_tlb` 共 19 处调用点，须逐点判定为「新建映射」还是「替换/覆盖映射」，仅前者可降级为本核 `invlpg`（不置位 `TLB_SHOOTDOWN_NEEDED`）。
  - 状态：[]
  - 详情：**实测更新（S-11 修复后，2/3/4 核）**：各 **42** 次（原 62），减少的 20 次即 4 个 IST × 5 页恒等映射调用消失（见 S-11 ③）——本项（map 路径过度 shootdown）**仍未处置**。**分类完成前不得改动**。判定准则——若目标 VA 此前**不存在**翻译，远程核不可能缓存该 VA 的陈旧条目（x86 不缓存"不存在"的翻译），无需远程失效；若为**替换**（典型：`huge split` 把 2MB PDE 换成 4KB 页表），远程核可能缓存旧的大页条目，**必须**失效，不可省。故本项不是"把 map 路径的 `flush_tlb` 一律去掉"，而是逐点判定。修复方式倾向：新增不置位的本核失效入口（如 `flush_tlb_local`）供"纯新建"调用点使用，`flush_tlb` 语义保持"本核 + 登记远程需求"不变。
...
- **注入 2**：临时把接收侧三步顺序改为"flush → 读代 → 声明"，确认门槛 7 的观测项**必须 FAIL**（假追平被检出）；验后完全还原。
  - **判别力（如实标注）**：**当前判别力有限**——"假追平"表现为跨核陈旧翻译，其后果需"远端核访问已 unmap 页、验证不再命中陈旧映射"的跨核探针才能稳定检出，而 `shootdown` 计数类弱观测检不出。按已裁定的「ii」路线，本工程**不建**跨核探针，跨核探针列为后续项；届时须补验。
...
- **D3: 用户态栈写入触发 `#PF`（本轮实测发现，未修）**：D1/D2 修复后用户态首次真正执行，实测 `[IDT] user exception: vec=14 err=0x7 rip=0x40003A rsp=0x7FFFFFFE8FF0 cr2=0x7FFFFFFE8FF6`（`err=0x7` = P=1|W=1|U=1 ⇒ 页存在但写保护；`cr2` 落在用户栈页 `0x7FFFFFFE8000` 内）。同一日志的 `[USER] SELF-CHECK: user_stack_rsp_page ... (unexpected: should be guard/unmapped)` 指向同一处。**本轮不修**：属独立的用户栈映射权限缺陷，须单独定位其 PTE 来源（fork 复制 / `map_user_page` 权限参数）。
  - 详情：**D5 修复后复跑（2/3/4 核，突然复现且位置稳定）**：三份日志各出现**一次** `[IDT] user exception: vec=14 err=0x7 rip=0x400048 cr2=<rsp+6>`（2 核 `rsp=0x7FFFFFF9EFF0`/`cr2=…EFF6`；3 核 `rsp=0x7FFFFFF01FF0`；4 核 `rsp=0x7FFFFFF6FFF0`），出现时机均为**第二次 `fork` 的前一个 `wait_pid` 返回之后**（紧跟首个 `destroy_page_table` 与 `[SMP] TLB shootdown`），此后进程继续执行并输出 `Y` ⇒ **该异常非终止性**（恢复机制未核实：可能是页表项被后续路径修正，也可能是硬件重试时权限已变，须单独定位）。与 `[USER] SELF-CHECK: user_stack_rsp_page ... (unexpected: should be guard/unmapped)` 指向同一处（用户栈顶页被映射为只读）。归属仍按上文：**独立缺陷，须单独定位 PTE 来源**；不属 D5 范围（D5 修复前该处直接挂起在内核态，无从观测到本异常）。
