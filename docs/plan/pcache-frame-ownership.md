...
2. **`pcache_get` 的 services re-export 零调用者**：`src/kernel/services/mm/pcache.rs` 的**五个**转发函数（`pcache_get` / `pcache_lookup` / `pcache_mark_dirty` / `pcache_put` / `pcache_invalidate_inode`）在 services 侧无调用点（`pcache_invalidate_inode` 的实际调用者是 framework 的 [handle.rs:178](../../src/kernel/framework/fs/vfs/handle.rs#L178)）。属 `eliminate-parallel-implementations.md` 范畴。**已由 [duplicate-impl-convergence.md](./duplicate-impl-convergence.md) D-5 承接处置**：该文件与其 `pub mod pcache;` 声明已删除（零调用者 + 能力等价入口存在 + 非 API/FFI/feature/硬件原语面），services 对 pcache 的唯一路径为 `framework::mm::pcache`。原文「六个」系笔误，实测为五个。
...
4. **旧帧归零释放的形态重复**：`frame_dec` 归零后的「x86_64 `defer_free` / aarch64 立即 `free_page`」形态在 [page_fault.rs:392-400](../../src/kernel/framework/mm/page_fault.rs#L392-L400)（MAP_PRIVATE COW 换叶）、[cow.rs:457-481](../../src/kernel/framework/mm/cow.rs#L457-L481)、[vmm_x86_64.rs:1443-1495](../../src/kernel/framework/mm/vmm_x86_64.rs#L1443-L1495) 与 [vmm_aarch64.rs:1248-1265](../../src/kernel/framework/mm/vmm_aarch64.rs#L1248-L1265) 各有一份。本工程按 D-2 口径**新增一份**（[pcache.rs `release_frame_after_last_holder`](../../src/kernel/framework/mm/pcache.rs)）——原因：该处被替换的是「同 VA 悬留的旧缓存帧」，与 acquire 的登记点同属 pcache 的持有模型，放在 pcache 内可保持「判据与释放同处一份」；D-3 **未删除** COW 路径的副本（那里注销的是 MAP_PRIVATE 换出时的映射持有，语义不同）。故形态重复由 4 处增至 5 处，收敛属 `eliminate-parallel-implementations.md` 范畴。**已由 [duplicate-impl-convergence.md](./duplicate-impl-convergence.md) D-1..D-4 承接收敛**：实测站点为 8 处（比本条目统计的 5 处更多），已全部收敛到 `framework/mm/mod.rs` 的双入口 `release_frame_locked`（要求已持 VMM_LOCK）/ `release_frame`（自持锁），本工程新增的 `release_frame_after_last_holder` 亦随之删除；同时修掉 `pcache::deref` 归零释放遗漏延迟释放的缺陷（该缺陷与本条目的形态重复同源，属预存问题）。
