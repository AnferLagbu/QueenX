...
- [ ] T6-T7：登记排后（TODO 33 项 / aarch64 编号）
  - 丙批审查整改（分册 9 **D-8** 余项 7 项）：**6 项已落地**——A3 OOMD victim 过滤僵尸（`93568c05`）/ A4 `terminated_count` 仅送达才累加（`4614e707`）/ B2 ext2 `i_ctime` 独立取值（`1b852e5d`）/ B4 换行（`3541b63c`）/ exfat 删静默成功覆写（`7079a94e`）/ F9 豁免残留清除（`29bee0a5`）；**A1 页表遍历竞态「闸门未过」**——`MmStruct` 无 per-process 归属（`Process` 仅 `cr3`、无 pid→mm 注册表、`CURRENT_MM` 唯一生产设置点为 exec），OOMD 跨进程遍历取不到 `&MmStruct`，原案不可达，待裁定后开工。
