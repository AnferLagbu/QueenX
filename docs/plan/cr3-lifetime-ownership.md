...
- **~~`vmspace.rs::protect()` 在新 dec 语义下的隐患~~（已由 `munmap-protect-pml4.md` 承接）**：[vmspace.rs:156-171](../../src/kernel/framework/vmspace.rs#L156-L171) 以"unmap → 同帧 remap"实现改权限。D-9④ 落地后 `unmap` 会 `frame_dec`（归零即 `defer_free`），故接入生产路径将出现"在用帧被注销并延迟释放后又被 remap"的 UAF 形态。当前该文件为零调用者脚手架（见上条），**未接入即无现网影响**；接入前须改为"就地改 PTE 权限"（同 `vmm::protect_page` 形态）。**承接结果**：该形态已消除——改为 `protect_page_in_table(self.pt_root, …)` 就地改权限（`munmap-protect-pml4.md` D-4），判别力由该文件 D-6 用例 `mm::vma_teardown::mprotect_targets_user_table` 承担（修复前形态实测变红）。
